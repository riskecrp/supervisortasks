...
export class DiscussionsService {
  ...
  async getAllDiscussions(): Promise<Discussion[]> {
    ...
    const supervisors = headers.slice(3).map(s => s?.toString().trim()).filter(Boolean); // Supervisors start from column D (index 3)
  ...
  async getSupervisorsFromDiscussions(): Promise<string[]> {
    ...
    return rows[0].slice(3).map(s => s?.toString().trim()).filter(Boolean);
  }
